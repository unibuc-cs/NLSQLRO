...
from dataset_generator.config import AppConfig
...
def validate_master_jsonl(
    ...
    """Validate all rows in a master JSONL file and return `(ok, summary)`."""
    ...
    with master_path.open("r", encoding="utf-8") as fh:
        for line_no, line in enumerate(fh, start=1):
            ...
            sql = str(row.get("sql", ""))
            task = normalize_task(row.get("task"), default="user")
            ...
            if task == "user" and strict_non_empty and result.row_count < min_rows:
                ok = False
                ...
                failures.append(
                    {
                        "line": line_no,
                        "id": row.get("id"),
                        "error": f"task={task} row_count={result.row_count} < min_rows={min_rows}",
                    }
                )
                continue

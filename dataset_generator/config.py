...
from dataset_generator.dataset_types import normalize_task
...
class ProviderConfig:
    ...
    translator_model: str = "google/madlad400-10b-mt"
    base_url: str = ""
    api_key_env: str = ""
...
class GenerationConfig:
    ...
    task_cycle: List[str] = field(default_factory=lambda: ["user", "operator"])
...
def load_config(config_path: Path) -> AppConfig:
    ...
    provider = ProviderConfig(
        ...
        translator_model=str(
            provider_payload.get("translator_model", "google/madlad400-10b-mt")
        ),
        base_url=str(provider_payload.get("base_url", "")),
        api_key_env=str(provider_payload.get("api_key_env", "")),
        ...
        translation_max_new_tokens=int(
            provider_payload.get("translation_max_new_tokens", 256)
        ),
        translation_num_beams=int(provider_payload.get("translation_num_beams", 4)),
        remote_params=(
            dict(provider_payload.get("remote_params", {}))
            if isinstance(provider_payload.get("remote_params", {}), dict)
            else {}
        ),
    )
    ...
    generation = GenerationConfig(
        ...
        task_cycle=[
            normalize_task(x, default="user")
            for x in generation_payload.get("task_cycle", ["user", "operator"])
        ],
    ...
    if not generation.task_cycle:
        generation.task_cycle = ["user"]
